...
-- filter _ Nil = Nil
-- filter p (head :. tail) = if p head then head :. filter p tail else filter p tail
filter p l = foldr (\a b -> if p a then a :. b else b) Nil l
...
reverse Nil = Nil
reverse l = foldl (\b el -> el :. b) Nil l
...
toListH :: List a -> [a]
toListH Nil = []
toListH l = foldr (\el b -> el : b) [] l

-- И обратно
fromListH :: [a] -> List a
fromListH [] = Nil
fromListH l = P.foldr (\el b -> el :. b) Nil l
...
intercalate :: [a] -> [[a]] -> [a]
intercalate _ [] = []
intercalate sep (head : tail) = P.foldl (\b el -> b ++ sep ++ el) head tail
